    ...
    return (
        <div className="flex  md:py-[40px] flex-row lg:grid lg:grid-cols-2  h-full">
            ...
            <div className="py-[20px] md:py-0 justify-center max-w-[653px] flex flex-col grow pl-[40px] lg:block  pr-[40px] md:pl-[20px] text-black">
                ...
                <h2 className="article-title font-bold line-clamp-3 md:line-clamp-5 mt-4 md:text-[1.44rem] lg:text-[1.728rem]"><span className="underline_span"><Link href={`/article/${encodeURIComponent(article?.attributes?.slug)}`}>{article?.attributes?.title}</Link></span></h2>
                <h4 className="hidden  mt-4 md:line-clamp-4">{article?.attributes?.description}</h4>
